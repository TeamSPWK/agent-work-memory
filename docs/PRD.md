...
12. ★ [Public Pages](#12-public-pages) — 랜딩·가격·가입·법무·회사·상태 (v2.1 신설)
...
> v2.1 신설(2026-05-11). v0.1 디자인 라운드는 *로그인 후 워크스페이스 내부* 23화면만
> 정의했고, PRD §5의 Core Concept도 *inside-app*만 다뤘다. 미가입자가 도달하는 외부
> 페이지(랜딩·가격·가입·법무·회사·상태)는 정의되지 않은 상태였다. 본 §12는 그 빈
> 자리를 채우고, 한국 SaaS 운영에 *법적으로 필수*인 페이지를 명문화한다.

### 12.1 카테고리

| # | 페이지 | 필수도 | 비고 |
|---|--------|--------|------|
| 1 | 랜딩 | **필수** | 미가입자 첫 접점. 인공지능기본법 위협 + 가치 + CTA |
| 2 | 가격 | **필수** | §7.2 티어 외부 노출. 트라이얼 진입 |
| 3 | 회원가입 | **차단성** | 이메일·Google OAuth. H4 온보딩 직전 |
| 4 | 로그인 | **차단성** | 비밀번호 / OAuth / 매직링크 |
| 5 | 비밀번호 재설정 | 필수 | 이메일 1회용 링크 |
| 6 | 이용약관 | **법적 필수** | 전자상거래법 |
| 7 | 개인정보처리방침 | **법적 필수** | K-PIPA / 인공지능기본법 §27 |
| 8 | 환불 정책 | **법적 필수** | 전자상거래법 (B2B SaaS도 명시) |
| 9 | 사업자 정보 | **법적 필수** | 통신판매업 신고, 사업자등록번호 (Spacewalk) |
| 10 | 회사 페이지 | 필수 | 1인 운영 투명 공개 (§11.3 연결) |
| 11 | 404 / 500 / 유지보수 | 필수 | Vercel 기본 가능, 브랜드 정합 |
| 12 | 상태 페이지 | 중간 | status.awm.spacewalk.tech — 1인 운영 가시성 |
| (v2.x) | 블로그·콘텐츠 마케팅 | 후속 | §8.4 콘텐츠 전략 결과물 둘 곳 |
| (v2.x) | 공개 docs / API 가이드 | 후속 | GitHub App / Workspace API Key |
| (v2.x) | PR kit · 데모 영상 | 후속 | D2~ 콘텐츠 채널 |

### 12.2 페이지별 핵심 메시지

**랜딩 (1)**
- Hero: *"AI가 만든 변경, 사람이 다시 설명할 수 있게."* — 인공지능기본법 §27 시행(2026-01-22)을 한 줄로 박는다.
- 가치 3블록: 회상(H1) · 감사(H2) · 1차 원인(H3) — v0.1 가설 띠와 동일 지표 노출.
- 사회적 증거: *PRD §1.4 외부 사고 사례 인용*(Replit Agent prod DB · PocketOS 9초 · 영국 학생 33% AI 작성). 가상 회사·인물명 금지.
- CTA: "5분 안에 워크스페이스 만들기" → H4 온보딩 진입.
- *1인 운영 투명 공개* 한 줄 fold-below.

**가격 (2)**
- 3 티어 (§7.2): Free · Team · Business. per-active-Operator 모델 노출.
- 디자인 파트너 5팀 한정 50% 할인 (§7.5) chip — 한정 수량 / 마감 *형식*만, 임의 기간 X.
- *Active Operator* 정의 1줄: "지난 30일 1회 이상 AI 작업이 기록된 사용자".
- 자동 세금계산서 + 토스페이먼츠 결제 명시.
- FAQ 5개: 미사용 환불 / Operator 외 멤버 / 인공지능기본법 자동 보고서 / 1인 운영 안정성 / 데이터 보존.

**회원가입·로그인 (3·4·5)**
- 이메일 + Google OAuth 2종으로 시작 (v2.x에 GitHub OAuth 추가 검토).
- 회원가입 직후 H4 화면 1(워크스페이스 생성)로 점프.
- 로그인 폼은 H4 toggleable.

**법무 4종 (6·7·8·9)**
- 한국어 원문 + 마지막 개정일.
- 환불 정책: 디자인 파트너 무료·트라이얼·정가 결제 각각 명시.
- 개인정보처리방침: *원문 transcript 미저장* 원칙을 명문화. 보존 기간 5년(인공지능기본법 §27 권고).
- 사업자 정보: Spacewalk 사업자등록번호 + 통신판매업 신고번호 + 대표 + 주소 + 고객문의 채널.
- 약관·정책 변경 시 *공지 후 30일* (전자상거래법 권고).

**회사 페이지 (10)**
- *1인 운영 투명 공개* — §11.3 위험 대응 명문화 위치.
- Spacewalk 사업자 소개·창업 맥락·1인 운영 정책·고객 지원 시간·번아웃·이탈 대비 정책.
- 디자인 파트너 계약·고객 페이지에서 *동일 카피* 인용 (PRD §3 페르소나 신뢰 기반).

**상태 페이지 (12)**
- status.awm.spacewalk.tech 또는 외부 SaaS(BetterStack·Status) integration.
- 다운타임·점검·인시던트 공지. 1인 운영의 *가시성* 부재 보완.
- v2.0 출시 직후 필수, MVP 단계는 *외부 SaaS link*도 OK.

### 12.3 법적 필수 페이지 (한국 SaaS 운영)

| 법령 | 의무 페이지 | 비고 |
|------|------------|------|
| 전자상거래법 | 이용약관·환불정책·사업자 정보 | 통신판매업 신고번호 노출 |
| K-PIPA (개인정보보호법) | 개인정보처리방침 | 수집 항목·보존 기간·제3자 제공·국외 이전 명시. Supabase Tokyo 데이터 거주국 표기 |
| 인공지능기본법 §27 | (개인정보처리방침 안에) AI 감사 로그 보존 정책 | 5년 권고 — Audit Export Settings(Settings 4)와 정합 |
| 정보통신망법 | 정보 수신자 명시 | 회사 페이지·footer |

> 법무 페이지 *문구*는 본 PRD에서 정의하지 않는다. 별도 법무 자문(`docs/areas/regulatory/`)을 거쳐 작성.

### 12.4 1인 운영 투명 공개 (PRD §11.3 연결)

§11.3에서 *"1인 운영 사실을 디자인 파트너 계약·고객 페이지에 명시"* 라고 명문화했다.
본 §12에서 둘 *위치*를 결정:

- **회사 페이지**: 한 페이지 분량의 솔직한 설명. *왜 1인인지, 어떤 자동화로 보완하는지, 무엇을 안 할 것인지*.
- **상태 페이지**: 1인 운영의 *가시성 부재*를 보완 — 점검·다운타임은 즉시 공개.
- **사업자 정보 footer**: 모든 페이지에 한 줄 노출.
- **랜딩 fold-below**: 가치 3블록 다음에 "Spacewalk 1인 창업자가 만들고 있습니다" 한 줄.

### 12.5 가설 + 측정 지표 (v0.1 hypothesis banner 형식 재사용)

| 페이지 그룹 | 가설 | 측정 |
|------------|------|------|
| **랜딩** | "30초 안에 핵심 가치 + CTA 클릭이 발생하면, 디자인 파트너 인터뷰 신청률이 ≥ 5%이다." | hero scroll-depth + CTA 클릭율 |
| **가격** | "티어 + Active Operator 정의 + 디자인 파트너 50% chip이 한 화면에 있으면, 트라이얼 시작률이 ≥ 15%이다." | 가격 → 가입 전환율 |
| **회원가입** | "이메일 + Google OAuth + 한국어 카피로, 가입 → H4 화면 1 도달까지 ≤ 90초이다." | 가입 폼 완료 시간 |
| **회사 페이지** | "1인 운영 투명 공개를 미리 노출하면, 디자인 파트너 계약 단계에서 *예상치 못한 우려*가 0건이다." | 계약 단계 이탈 사유 |

### 12.6 운영 룰 (§9 · `.claude/rules/prd-and-strategy-collaboration.md` 적용)

- **가상 회사·인물명 금지** — 랜딩의 사회적 증거는 *PRD §1.4 외부 보도 사례*만. 디자인 파트너 5팀은 사후에 *동의받은 사례*만 실명 인용.
- **익명화 워크스페이스** — 가격 페이지·랜딩 데모 mock은 v0.1과 동일한 *"B2B SaaS 28명 / D2C 이커머스 35명 / 솔로 인디 1명"* 식.
- **임의 기간 milestone 금지** — 가격 페이지의 디자인 파트너 할인은 *팀 수 한정*(예: "선착순 5팀")이지 *기간 한정*(X)이 아니다.
- **결제 직군 편견 금지** — 랜딩·가격은 *역할*(Operator/Reviewer/Admin)로만 분류, *개발/비개발* 이분 금지.
- **1인 운영 sustainability** — 24/7 지원·enterprise SLA·전화 응답 카피 X. 영업시간 채널톡·자동 응답 기반.

### 12.7 Exit Criteria

§12 완료 = 다음 모두 PASS:

1. v2.1 § 12 본문 작성 완료 (본 문서)
2. 외부 페이지 12종 디자인 라운드(`docs/projects/plans/p0-design-v0.2/`) — 카테고리 #1~#11 시각 시안
3. m2.5 Plan(`docs/projects/plans/m2.5-public-pages-from-design.md`) 코드 이식 단계 정의
4. 법무 4종(약관·정책·환불·사업자) *문구* 별도 자문 트래킹 시작 (`docs/areas/regulatory/legal-pages.md`)
5. 랜딩 / 가격 / 회원가입 화면이 H4 온보딩과 *깨지지 않게* 연결 (라우터 분리 + 세션 핸드오프)

### 12.8 v2.x로 미루는 항목

| 항목 | 미루는 사유 |
|------|------------|
| 블로그 / 콘텐츠 마케팅 인프라 | §8.4 콘텐츠 채널 결정 후. CMS 선택(MDX·Notion·Sanity) 별도 자문 |
| 공개 API 문서 / GitHub App 가이드 | API 안정화 후. m2~m2.5에서 변경 가능성 큼 |
| PR kit · 데모 영상 | D2 결제 검증 후. 제작 비용 vs 효용 검증 필요 |
| 다국어 (영어·일본어) | D3 글로벌 진입 시 (§8.7) |
| 본인 인증 (PASS / 휴대폰 인증) | v2.0 결제 트리거 검증 후 — 본인 인증이 가입 전환율을 떨어뜨릴 가능성 검증 |
